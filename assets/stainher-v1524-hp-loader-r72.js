/* Stainher V15.24 · R97 · puente anti-cache HP + Presupuestos + Liderazgo + Sistema + Firmas + Informes + Forecast.
...
(()=>{
  'use strict';
  if(window.__STAINHER_HP_LOADER_VERSION__==='R97')return;
  window.__STAINHER_HP_LOADER_R72__=true;
  window.__STAINHER_HP_LOADER_VERSION__='R97';
  ...
  async function refreshContractForecast(){
    try{
      ...
      await load('stainher-contract-forecast-runtime-r97','stainher-v1524-contract-forecast-r97.js');
      window.StainherContractForecastR95?.install?.();
      window.StainherContractForecastR96?.install?.();
      window.StainherContractForecastR97?.install?.();
      window.dispatchEvent(new CustomEvent('stainher:contract-forecast-r97-ready'));
    }catch(error){
  ...
  window.StainherHPR97={refresh,refreshBudgets,refreshLeadership,refreshSystem,refreshSignature,refreshFreeReport,refreshContractForecast};
  refreshBudgets();
  refreshLeadership();
  refreshSystem();
  refreshSignature();
