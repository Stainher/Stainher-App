...
(()=>{
  ...
  function ensureCustomControlEditor(){
    if(window.__STAINHER_CUSTOM_CONTROL_BUILDER_V1__||document.getElementById('stainher-v1524-custom-control-builder-live'))return;
    const script=document.createElement('script');
    script.id='stainher-v1524-custom-control-builder-live';
    script.src='stainher-v1524-custom-control-builder.js?build=20260911-r30-edit-published-controls';
    script.async=false;
    script.addEventListener('error',()=>console.error('No se pudo cargar el editor de Controles Stainher.'),{once:true});
    document.head.appendChild(script);
  }
  function ensureVacation7x7Rule(){
    if(window.__STAINHER_VACATION_7X7_RULE__||document.getElementById('stainher-v1524-vacation-7x7-rule-live'))return;
    const script=document.createElement('script');
    script.id='stainher-v1524-vacation-7x7-rule-live';
    script.src='stainher-v1524-vacation-7x7-rule.js?build=20260911-r31-rrhh-business-days';
    script.async=false;
    script.addEventListener('error',()=>console.error('No se pudo cargar la regla actualizada de vacaciones 7x7.'),{once:true});
    document.head.appendChild(script);
  }
  function ensureLeadershipLightContrast(){
    if(window.__STAINHER_LEADERSHIP_LIGHT_CONTRAST__||document.getElementById('stainher-v1524-leadership-light-contrast-live'))return;
    const script=document.createElement('script');
    script.id='stainher-v1524-leadership-light-contrast-live';
    script.src='stainher-v1524-leadership-light-contrast.js?build=20260911-r32-light-contrast';
    script.async=false;
    script.addEventListener('error',()=>console.error('No se pudo cargar el contraste claro de formularios Liderazgo.'),{once:true});
    document.head.appendChild(script);
  }
  function ensureHomeVisibilityFix(){
  ...
  function ensureLeadershipDeleteVerify(){
    if(window.__STAINHER_LEADERSHIP_DELETE_VERIFY__||document.getElementById('stainher-v1524-leadership-delete-verify-live'))return;
    const script=document.createElement('script');
    script.id='stainher-v1524-leadership-delete-verify-live';
    script.src='stainher-v1524-leadership-delete-verify.js?build=20260914-r45-verified-delete';
    script.async=false;
    script.addEventListener('error',()=>console.error('No se pudo cargar la verificación de eliminación de Liderazgo.'),{once:true});
    document.head.appendChild(script);
  }
  function snapshot(){
  ...
  function install(){ensureCustomControlEditor();ensureVacation7x7Rule();ensureLeadershipLightContrast();ensureHomeVisibilityFix();ensureLeadershipDeleteVerify();snapshot();const base=window.renderSistema;if(typeof base==='function'&&!base.__stainherRuntimeAudit){const wrapped=async function(){const out=await base.apply(this,arguments);enhanceSystem();return out};wrapped.__stainherRuntimeAudit=true;window.renderSistema=wrapped}enhanceSystem();window.addEventListener('stainher:modules-ready',()=>{ensureCustomControlEditor();ensureVacation7x7Rule();ensureLeadershipLightContrast();ensureHomeVisibilityFix();ensureLeadershipDeleteVerify();snapshot()});window.addEventListener('stainher:modules-error',snapshot)}
