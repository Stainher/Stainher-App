...
(()=>{
  ...
  async function addBalanceToAccount(){
    ...
    const duplicates=[...modal.querySelectorAll('#accountVacationBalance,.stainher-account-vacation')];
    if(duplicates.length){duplicates.slice(1).forEach(card=>card.remove());return}
