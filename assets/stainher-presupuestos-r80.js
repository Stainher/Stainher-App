/* Stainher V15.24 · R110 · Presupuestos técnico comerciales
 * Administración del Contrato -> Presupuestos
 * Código: NOMBRE-EQUIPO-PE-DD-MM-AA
 */
(()=>{
  'use strict';
  const BUILD='20260924-r110-presupuestos-confiabilidad';
  ...
  const canDelete=()=>canUse()&&role()==='administrador';
  const canEdit=row=>canUse()&&(role()==='administrador'||String(row?.created_by||'')===uid());
  ...
  function shell(){
    const c=document.getElementById('contractContent');if(!c)return;c.innerHTML=`<div class="r80-toolbar"><div><h3 style="margin:0 0 3px">Presupuestos técnico comerciales</h3><div class="muted">Histórico compartido · PDF con firma institucional del Administrador.</div></div>${canUse()?'<button class="btn primary" type="button" data-r80-new>+ Nuevo presupuesto</button>':''}</div><div id="r80Body"><div class="empty">Cargando presupuestos…</div></div>`;c.querySelector('[data-r80-new]')?.addEventListener('click',()=>openEditor())
  }
  ...
  function renderList(){
    ...
    body.innerHTML=`<div class="r80-kpis"><div class="r80-kpi"><span>Presupuestos</span><b>${rows.length}</b></div><div class="r80-kpi"><span>Emitidos</span><b>${issued}</b></div><div class="r80-kpi"><span>Valor total</span><b>${money(total)}</b></div><div class="r80-kpi"><span>Documento</span><b>PE · PDF</b></div></div><div class="r80-list"><table><thead><tr><th>Código</th><th>Equipo</th><th>Fecha</th><th>Estado</th><th style="text-align:right">Venta neta</th><th style="text-align:right">IVA</th><th style="text-align:right">Total</th><th style="text-align:right">Acciones</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${esc(r.codigo)}</b></td><td>${esc(r.equipo_nombre)}</td><td>${esc(dateCL(r.fecha))}</td><td>${esc(r.estado||'borrador')}</td><td class="money">${money(r.venta_neta)}</td><td class="money">${money(r.iva)}</td><td class="money"><b>${money(r.total_final)}</b></td><td><div class="actions">${canEdit(r)?`<button class="btn" data-edit="${esc(r.id)}">Editar</button>`:''}<button class="btn" data-pdf="${esc(r.id)}">PDF</button>${canDelete()?`<button class="btn danger-btn" data-delete="${esc(r.id)}">Eliminar</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="8"><div class="empty">Aún no hay presupuestos registrados.</div></td></tr>'}</tbody></table></div>`;
  ...
  function openEditor(id){const row=id?state().rows.find(x=>String(x.id)===String(id)):null;if(!canUse()||(row&&!canEdit(row)))return window.toast?.('No tienes permiso para editar este presupuesto.','error');state().editing=row?clone(row):blank();renderEditor()}
  ...
  async function saveBudget(){
    const d=readDraft(),existing=d.id?state().rows.find(x=>String(x.id)===String(d.id)):null;if(!canUse()||(existing&&!canEdit(existing)))return window.toast?.('No tienes permiso para guardar este presupuesto.','error');if(!d.equipo_id||!d.equipo_nombre)return window.toast?.('Selecciona un equipo.','error');if(!d.fecha)return window.toast?.('Selecciona la fecha.','error');const k=calc(d),payload={codigo:codeFor(d.equipo_nombre,d.fecha),equipo_id:d.equipo_id,equipo_nombre:d.equipo_nombre,fecha:d.fecha,materiales:d.materiales,fletes:d.fletes,utilidad_pct:d.utilidad_pct,...k,observacion:d.observacion,estado:d.estado,updated_at:new Date().toISOString()};let out;if(d.id)out=await window.sb.from(TABLE).update(payload).eq('id',d.id).select('*').single();else out=await window.sb.from(TABLE).insert({...payload,created_by:uid()}).select('*').single();if(out.error)return window.toast?.(out.error.code==='23505'?'Ya existe un presupuesto para ese equipo y fecha.':out.error.message||String(out.error),'error');window.toast?.('Presupuesto guardado correctamente.','success');state().editing=null;await loadRows();
  }
  async function removeBudget(id){if(!canDelete())return window.toast?.('Solo el Administrador puede eliminar presupuestos.','error');if(!window.confirm?.('¿Eliminar este presupuesto?'))return;const out=await window.sb.from(TABLE).delete().eq('id',id);if(out.error)return window.toast?.(out.error.message||String(out.error),'error');window.toast?.('Presupuesto eliminado.','success');await loadRows()}
  async function administratorSignature(){
    const cached=state().administratorSignature;if(cached)return cached;
    const out=await window.sb.rpc('presupuesto_firma_administrador_v110');
    if(out.error)throw out.error;
    const signer=Array.isArray(out.data)?out.data[0]:out.data;
    if(!signer?.imagen_png)throw new Error('El Administrador no tiene una firma institucional guardada.');
    state().administratorSignature=signer;return signer;
  }
  ...
  async function makePdf(d,markIssued){
    if(!canUse())return window.toast?.('No tienes permiso para generar presupuestos.','error');if(!d.equipo_nombre||!d.fecha)return window.toast?.('Selecciona equipo y fecha antes de generar el PDF.','error');let signer;try{signer=await administratorSignature()}catch(error){return window.toast?.(error.message||String(error),'error')}const C=typeof window.ensurePdf==='function'?window.ensurePdf():window.jspdf?.jsPDF;if(!C)return window.toast?.('No se pudo cargar el generador PDF.','error');const k=calc(d),code=codeFor(d.equipo_nombre,d.fecha),doc=new C({orientation:'portrait',unit:'mm',format:'a4'});
  ...
  function install(){
    mountStyle();
    if(typeof window.renderContractTab==='function'&&!window.renderContractTab.__r110){const base=window.renderContractTab,wrapped=function(){mountTab();if(window.state?.contractTab==='presupuestos')return renderBudgetTab();const r=base.apply(this,arguments);requestAnimationFrame(mountTab);return r};wrapped.__r110=true;wrapped.__base=base;window.renderContractTab=wrapped}
    if(typeof window.renderContrato==='function'&&!window.renderContrato.__r110){const base=window.renderContrato,wrapped=function(){const r=base.apply(this,arguments);requestAnimationFrame(()=>{mountTab();if(window.state?.contractTab==='presupuestos')renderBudgetTab()});return r};wrapped.__r110=true;wrapped.__base=base;window.renderContrato=wrapped}
