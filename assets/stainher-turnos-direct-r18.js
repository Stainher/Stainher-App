...
(function installStainherTurnosDirectR18(){
  ...
  const BUILD='20260909-r31-calendar-groups-hf';
  ...
  const CODES={encierro_planificado:'ET',encierro_no_planificado:'EF',suspendido_encierro:'SE',dia_adicional:'DA',hora_extra:'HE',feriado:'HF',vacaciones:'V',licencia_medica:'LM',permiso:'P',falta:'F',capacitacion:'CAP',otro:'Otr',encierro:'ENC'};
  let seq=0;
  ...
  const code=t=>CODES[String(t||'')]||window.v1520TurnTypeShort?.(t)||'Otr';
  ...
  function matrixGroupHtml(d,idx,y,m,edit){
    ...
    return `<div class="r18-matrix r18-desktop"><table><thead><tr><th>Colaborador</th>${head}</tr></thead><tbody>${rows||'<tr><td class="empty">Sin personas en la malla.</td></tr>'}</tbody></table></div>`;
  }
  ...
  function bind(page,d,edit){
    ...
    page.querySelector('[data-r18-today]')?.addEventListener('click',()=>{const now=new Date();window.state.turnYearV1512=now.getFullYear();window.state.turnMonthV1512=now.getMonth()+1;render()});
    page.querySelector('[data-r18-group]')?.addEventListener('change',e=>{window.state.turnGroupV1512=e.target.value;render()});
    page.querySelectorAll('[data-r18-tab]').forEach(btn=>btn.addEventListener('click',()=>{window.state.v1520TurnTab=btn.dataset.r18Tab;render()}));
    page.querySelector('[data-r18-new-event]')?.addEventListener('click',()=>window.v1520OpenTurnEvent?.());
    page.querySelector('[data-r18-report]')?.addEventListener('click',()=>window.v1516OpenTurnMonthlyReport?.());
    ...
    page.querySelectorAll('[data-r18-edit-event]').forEach(btn=>btn.addEventListener('click',()=>window.v1520OpenTurnEvent?.(btn.dataset.r18EditEvent)));
    page.querySelectorAll('[data-r18-delete-event]').forEach(btn=>btn.addEventListener('click',()=>window.v1520DeleteTurnEvent?.(btn.dataset.r18DeleteEvent)));
  }

  async function render(){
    const page=document.getElementById('page-turnos');if(!page||!window.canViewV11?.('turnos'))return;
    const token=++seq;page.innerHTML='<div class="empty">Cargando programación de turnos…</div>';
    try{
      const d=await window.v1520LoadTurnData();if(token!==seq)return;
      window.state.v1520TurnData=d;window.state.v1512TurnData=d;
      const y=Number(window.state.turnYearV1512||new Date().getFullYear()),m=Number(window.state.turnMonthV1512||new Date().getMonth()+1),idx=makeIndexes(d,y,m),edit=canEdit();
      const ref=iso(y,m,Math.min(new Date().getDate(),daysIn(y,m))),cov=coverage(d,idx,ref);
      const inside=[...idx.inside].length,outside=[...idx.outside].length,suspended=[...idx.suspended].length;
      const groups=(d.groups||[]).map(([id,name])=>`<option value="${esc(id)}" ${String(window.state.turnGroupV1512||'')===String(id)?'selected':''}>${esc(name)}</option>`).join('');
      const tab=['malla','novedades'].includes(String(window.state.v1520TurnTab||''))?String(window.state.v1520TurnTab):'malla';window.state.v1520TurnTab=tab;
      page.innerHTML=`<div class="topbar"><div></div><div class="actions"></div></div><div class="r18-turn-meta"><span class="status ${d.publication?.draft?'warn':'ok'}">${d.publication?.draft?'Borradores pendientes':'Malla publicada'}</span><span>${esc(monthName(m))} ${y}</span></div><div class="r18-turn-toolbar"><button class="btn" data-r18-prev>‹</button><div class="r18-month">${esc(monthName(m))} ${y}</div><button class="btn" data-r18-next>›</button><label>Grupo<select class="field" data-r18-group><option value="">Todos</option>${groups}</select></label></div><div class="r18-turn-actions">${edit?'<button class="btn" data-r18-template>↓ Plantilla anual</button><button class="btn" data-r18-import>↑ Importar Excel</button><button class="btn primary" data-r18-new-event>+ Nuevo evento</button><button class="btn positive" data-r18-publish>Publicar malla</button>':''}<button class="btn" data-r18-report>↓ Informe mensual</button><button class="btn" data-r18-today>Mes actual</button></div><div class="r18-kpis"><div class="r18-kpi"><span>Programados · ${esc(fmtDate(ref))}</span><b>${cov.scheduled}</b></div><div class="r18-kpi"><span>Disponibles efectivos</span><b>${cov.available}</b></div><div class="r18-kpi"><span>Ausentes programados</span><b>${cov.absent}</b></div><div class="r18-kpi"><span>Cobertura operativa</span><b>${cov.pct.toFixed(1)}%</b></div><div class="r18-kpi"><span>Personal visible</span><b>${(d.people||[]).length}</b></div></div><div class="r18-kpis r18-kpis-secondary"><div class="r18-kpi"><span>Encierro dentro de turno</span><b>${inside}</b></div><div class="r18-kpi"><span>Encierro fuera de turno</span><b>${outside}</b></div><div class="r18-kpi"><span>Suspendido por encierro</span><b>${suspended}</b></div></div><div class="r18-turn-tabs"><button class="btn ${tab==='malla'?'active':''}" data-r18-tab="malla">Malla</button><button class="btn ${tab==='novedades'?'active':''}" data-r18-tab="novedades">Eventos y novedades</button></div><div data-r18-content>${tab==='malla'?matrixHtml(d,idx,y,m,edit)+mobileHtml(d,idx,y,m,edit):eventsHtml(d,idx,edit)}</div>`;
      bind(page,d,edit);
    }catch(error){if(token===seq)page.innerHTML=`<div class="notice error">${esc(error?.message||String(error))}</div>`}
  }

  function step(delta){let y=Number(window.state.turnYearV1512),m=Number(window.state.turnMonthV1512)+Number(delta);while(m<1){m+=12;y--}while(m>12){m-=12;y++}window.state.turnYearV1512=y;window.state.turnMonthV1512=m;render()}

  function boot(){
    if(!window.state)return setTimeout(boot,60);
    const current=window.renderTurnosV15;if(typeof current!=='function')return setTimeout(boot,60);
    window.renderTurnosV15=render;window.renderTurnosV15.__stainherDirectR18=true;window.STAINHER_TURNOS_DIRECT={build:BUILD,ready:true};
  }
  boot();
})();
