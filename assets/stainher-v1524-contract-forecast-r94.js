/* Stainher V15.24 · R94 · Forecast EDP fresco + contraste de gráficos + alineación historial EDP.
 * No modifica fórmulas del Forecast ni lógica de login/sesión.
 */
(()=>{
  'use strict';
  const BUILD='20260917-r94-forecast-edp-colors';
  if(window.__STAINHER_CONTRACT_FORECAST_R94__===BUILD)return;
  window.__STAINHER_CONTRACT_FORECAST_R94__=BUILD;

  function installStyle(){
    if(document.getElementById('stainher-contract-r94-style'))return;
    const s=document.createElement('style');
    s.id='stainher-contract-r94-style';
    s.textContent=`
  ...
  function wrapForecast(){
    ...
    if(typeof current!=='function'||current.__r94)return false;
    const wrapped=async function(){
      const year=Number(document.getElementById('forecastYear')?.value||window.state?.forecastYear||new Date().getFullYear());
      try{await refreshEdp(year)}catch(error){console.error('[Stainher Forecast R94] No fue posible refrescar EDP',error)}
      const out=await current.apply(this,arguments);
      requestAnimationFrame(styleForecastCharts);
      return out;
    };
    wrapped.__r94=true;wrapped.__base=current;window.loadForecastV9=wrapped;
    return true;
  }
  ...
  window.StainherContractForecastR94=Object.freeze({install,refreshEdp,styleForecastCharts});
})();
