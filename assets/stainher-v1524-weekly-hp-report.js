...
(()=>{
  ...
  const state={year:new Date().getFullYear(),month:new Date().getMonth()+1,periods:[],people:[],profiles:new Map(),malla:[],nov:[],adjust:[],classes:new Map(),rows:[]};
  ...
  async function load(){
    ...
    const [dot,prof,mal,nov,adj,cls]=await Promise.all([
    ...
    state.people=(dot.data||[]).filter(x=>x.user_id);state.profiles=new Map((prof.data||[]).map(x=>[String(x.id),x]));state.malla=mal.data||[];state.nov=nov.data||[];state.adjust=adj.error?[]:(adj.data||[]);state.classes=new Map((cls.error?[]:cls.data||[]).map(x=>[String(x.user_id),x.clasificacion]));
  ...
  function calcPeriod(person,p){
    const prof=state.profiles.get(String(person.user_id))||{},r=String(prof.rol||'').toLowerCase();let admin=0,oper=0;
    if(['administrador','confiabilidad','gerente'].includes(r))admin=manual(person.user_id,'terreno_administrativo',p.fecha_inicio,p.fecha_fin);
      ...
      for(let d=p.fecha_inicio;d<=p.fecha_fin;d=dplus(d,1)){if(blocked(person.user_id,d))continue;const t=turn(person.user_id,d);if(!t||t==='L')continue;const dow=new Date(d+'T12:00:00').getDay();if(dow>=1&&dow<=3)admin+=12;else if(dow===4)admin+=6}
    }else{
      for(let d=p.fecha_inicio;d<=p.fecha_fin;d=dplus(d,1)){if(blocked(person.user_id,d))continue;const td=turn(person.user_id,d),prev=turn(person.user_id,dplus(d,-1));if(td==='A')oper+=12;if(td==='C')oper+=5;if(prev==='C'&&!blocked(person.user_id,d))oper+=7}
    }
    return {admin,oper,spor:manual(person.user_id,'operativa_esporadica',p.fecha_inicio,p.fecha_fin)};
  }
  function calcPerson(person){const periods=state.periods.map(p=>calcPeriod(person,p)),admin=sum(periods,'admin'),oper=sum(periods,'oper'),spor=sum(periods,'spor');return {...person,clasificacion:state.classes.get(String(person.user_id))||'operaciones',periods,admin,oper,spor,total:admin+oper+spor}}

  function monthlySummary(){
    const group=key=>{const rows=state.rows.filter(r=>r.clasificacion===key),admin=sum(rows,'admin'),oper=sum(rows,'oper'),spor=sum(rows,'spor'),faena=admin+oper+spor;return {rows,admin,oper,spor,faena,fte:rows.filter(r=>r.total>0).length}};
    const operaciones=group('operaciones'),inversiones=group('inversiones'),grand=operaciones.faena+inversiones.faena;
    operaciones.pct=grand?operaciones.faena/grand*100:0;inversiones.pct=grand?inversiones.faena/grand*100:0;return {operaciones,inversiones,grand};
  }
  function summaryHtml(){const s=monthlySummary(),fmt=n=>Number(n||0).toLocaleString('es-CL',{maximumFractionDigits:2}),pct=n=>`${Number(n||0).toLocaleString('es-CL',{maximumFractionDigits:1})}%`;return `<div class="hp-summary-wrap"><table class="hp-summary"><thead><tr><th colspan="3">RESUMEN MENSUAL</th></tr><tr><th></th><th>Operaciones</th><th>Inversiones</th></tr></thead><tbody><tr><th>TOTAL HH Administrativas</th><td>${fmt(s.operaciones.admin)}</td><td>${fmt(s.inversiones.admin)}</td></tr><tr><th>TOTAL HH Operativas</th><td>${fmt(s.operaciones.oper)}</td><td>${fmt(s.inversiones.oper)}</td></tr><tr><th>TOTAL HH Esporádicas</th><td>${fmt(s.operaciones.spor)}</td><td>${fmt(s.inversiones.spor)}</td></tr><tr class="hp-summary-strong"><th>TOTAL HH EN FAENA</th><td>${fmt(s.operaciones.faena)}</td><td>${fmt(s.inversiones.faena)}</td></tr><tr><th>TOTAL HH</th><td>${fmt(s.operaciones.faena)}</td><td>${fmt(s.inversiones.faena)}</td></tr><tr><th>%</th><td>${pct(s.operaciones.pct)}</td><td>${pct(s.inversiones.pct)}</td></tr><tr class="hp-summary-strong"><th>Total FTE</th><td>${s.operaciones.fte}</td><td>${s.inversiones.fte}</td></tr></tbody></table></div>`}

  function renderTable(){const head1=state.periods.map(p=>`<th colspan="3">${esc(periodLabel(p))}</th>`).join(''),head2=state.periods.map(()=>'<th>Horas Administrativas</th><th>Horas Operativas</th><th>Horas Operativas Esporádicas</th>').join(''),body=state.rows.map(r=>`<tr><td>${CONTRACT}</td><td>${esc((r.cargo||'').toUpperCase())}</td><td>${esc(r.rut||'')}</td><td>${esc(r.nombre)}</td>${r.periods.map(x=>`<td>${x.admin||0}</td><td>${x.oper||0}</td><td>${x.spor||0}</td>`).join('')}</tr>`).join('');return `<div class="hp-table-wrap"><table class="hp-table"><thead><tr><th rowspan="2">Número Contrato</th><th rowspan="2">Gerencia Origen</th><th rowspan="2">Rut Trabajador</th><th rowspan="2">Nombre Trabajador</th>${head1}</tr><tr>${head2}</tr></thead><tbody>${body}</tbody></table></div>`}
  ...
  async function saveClassification(e){const el=e.currentTarget,o={user_id:el.dataset.user,anio:state.year,mes:state.month,clasificacion:el.value,created_by:window.state.session.user.id,updated_at:new Date().toISOString()};const {error}=await window.sb.from('hp_clasificacion_personal').upsert(o,{onConflict:'user_id,anio,mes'});if(error){window.toast?.(error.message,'error');return render()}window.toast?.('Clasificación HP actualizada.','success');await render()}
  function exportExcel(){if(!window.XLSX)return window.toast?.('No está disponible el exportador Excel.','error');const detail=state.rows.map(r=>{const o={'Número Contrato':CONTRACT,'Gerencia Origen':(r.cargo||'').toUpperCase(),'Rut Trabajador':r.rut||'','Nombre Trabajador':r.nombre,'Clasificación':r.clasificacion==='inversiones'?'Inversiones':'Operaciones'};state.periods.forEach((p,i)=>{const l=periodLabel(p),x=r.periods[i];o[`${l} · Horas Administrativas`]=x.admin;o[`${l} · Horas Operativas`]=x.oper;o[`${l} · Horas Operativas Esporádicas`]=x.spor});return o});const s=monthlySummary(),summary=[['RESUMEN MENSUAL','Operaciones','Inversiones'],['TOTAL HH Administrativas',s.operaciones.admin,s.inversiones.admin],['TOTAL HH Operativas',s.operaciones.oper,s.inversiones.oper],['TOTAL HH Esporádicas',s.operaciones.spor,s.inversiones.spor],['TOTAL HH EN FAENA',s.operaciones.faena,s.inversiones.faena],['TOTAL HH',s.operaciones.faena,s.inversiones.faena],['%',s.operaciones.pct/100,s.inversiones.pct/100],['Total FTE',s.operaciones.fte,s.inversiones.fte]];const wb=XLSX.utils.book_new(),ws1=XLSX.utils.json_to_sheet(detail),ws2=XLSX.utils.aoa_to_sheet(summary);ws2['B7'].z='0%';ws2['C7'].z='0%';XLSX.utils.book_append_sheet(wb,ws1,'HP Trabajador');XLSX.utils.book_append_sheet(wb,ws2,'Resumen Mensual');XLSX.writeFile(wb,`Reporte_Semanal_HP_${state.year}_${String(state.month).padStart(2,'0')}.xlsx`)}
