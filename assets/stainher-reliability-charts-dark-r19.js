...
(()=>{
  ...
  function palette(id,light){
    if(id==='chartHours')return light?{fill:'#72b79f',border:'#367f6b'}:{fill:'#5ee0b5',border:'#b4f8df'};
    if(id==='chartTrend')return light?{fill:'rgba(49,112,181,.18)',border:'#316fae'}:{fill:'rgba(103,190,255,.20)',border:'#7dd3fc'};
    return light?{fill:'#73a9d8',border:'#376f9e'}:{fill:'#67b7f7',border:'#bae3ff'};
  }
  function styleChart(canvas){
    const chart=chartFor(canvas);if(!chart)return false;
    const light=theme()==='light',colors=palette(canvas.id,light);
  ...
  function refresh(){
    let pending=false;
    IDS.forEach(id=>{const canvas=document.getElementById(id);if(canvas&&!styleChart(canvas))pending=true});
    return pending;
  }
  function schedule(){
    clearTimeout(timer);
    requestAnimationFrame(()=>refresh());
    timer=setTimeout(()=>{refresh();setTimeout(refresh,240)},90);
  }
