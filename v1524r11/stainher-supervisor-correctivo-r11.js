...
(function installSupervisorCorrectivoHistoryR11(){
  ...
  function installStyle(){
    ...
    const s=document.createElement('style');s.id='stainher-supervisor-history-final-r11-style';
    s.textContent=`
  ...
  function renameHeading(heading){
    if(!heading||/Historial del per[ií]odo correctivo/i.test(txt(heading)))return;
  ...
  function findHistory(){
    ...
    if(!table){table=[...page.querySelectorAll('table')].find(t=>{const hs=[...t.querySelectorAll('thead th')].map(txt).join('|');return /Fecha/i.test(hs)&&/Equipo/i.test(hs)&&(/Duraci/i.test(hs)||/Estado/i.test(hs))})||null;panel=table?.closest?.('.panel,details,section,article')||table?.parentElement||null}
    return table&&panel?{page,panel,table,heading}:null;
  }

  function cleanKpiSummary({panel,heading}){
    [...panel.children].forEach(el=>{if(el!==heading&&/evento\(s\).*excluido\(s\).*KPI/i.test(txt(el)))el.classList.add('stainher-supervisor-kpi-summary-r11')});
    [...panel.querySelectorAll('span,small,div')].forEach(el=>{if(/evento\(s\).*excluido\(s\).*KPI/i.test(txt(el))&&el.children.length===0)el.classList.add('stainher-supervisor-kpi-summary-r11')});
  }
  ...
  function renderFinalCards(){
    ...
    if(!isSupervisor()){page.classList.remove('stainher-supervisor-history-final-r11');page.querySelectorAll('.stainher-supervisor-history-cards-r11').forEach(x=>x.remove());return}
    page.classList.add('stainher-supervisor-history-final-r11');
    const found=findHistory();if(!found)return;
    renameHeading(found.heading);cleanKpiSummary(found);
    if(!isMobile())return;
    const {panel,table}=found,headers=[...table.querySelectorAll('thead th')].map(txt);if(!headers.length)return;
    ...
    const old=panel.querySelector(':scope > .stainher-supervisor-history-cards-r11');if(old&&signature===lastSignature)return;
    rendering=true;
    try{
      old?.remove();const cards=document.createElement('div');cards.className='stainher-supervisor-history-cards-r11';
      ...
      rows.forEach(row=>{
        const cells=[...row.children];if(cells.length===1&&/Sin atenciones|Sin averías/i.test(txt(row)))return;
        const get=key=>allowed.find(x=>norm(x.label).includes(norm(key)))?.i,ei=get('equipo'),fi=get('fecha');
        ...
        const b=document.createElement('b');b.textContent=ei!=null?txt(cells[ei])||'Avería':'Avería';const d=document.createElement('span');d.textContent=fi!=null?txt(cells[fi]):'';head.append(b,d);card.appendChild(head);
        allowed.forEach(({label,i})=>{if(i===ei||i===fi||!cells[i])return;const field=document.createElement('div');field.className='stainher-supervisor-history-field-r11';const lab=document.createElement('div');lab.className='stainher-supervisor-history-label-r11';lab.textContent=label;const val=document.createElement('div');val.className='stainher-supervisor-history-value-r11';if(/Estado final/i.test(label)){const status=cells[i].querySelector('.status');if(status)val.appendChild(status.cloneNode(true));else val.textContent=txt(cells[i])}else val.textContent=txt(cells[i])||'—';field.append(lab,val);card.appendChild(field)});
  ...
  function boot(){installStyle();renderFinalCards();const root=document.getElementById('page-correctivo')||document.body;let pending=false;new MutationObserver(()=>{if(rendering||pending)return;pending=true;queueMicrotask(()=>{pending=false;renderFinalCards()})}).observe(root,{childList:true,subtree:true,characterData:true});window.addEventListener('resize',renderFinalCards,{passive:true});let n=0;const timer=setInterval(()=>{renderFinalCards();if(++n>40)clearInterval(timer)},100)}
