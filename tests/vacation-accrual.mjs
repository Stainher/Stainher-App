...
const db=new PGlite();
const admin='00000000-0000-0000-0000-000000000001', user='00000000-0000-0000-0000-000000000002', other='00000000-0000-0000-0000-000000000003', negative='00000000-0000-0000-0000-000000000004';
...
insert into perfiles values('${admin}','Admin','administrador',true,15),('${user}','Juan','confiabilidad',true,2.46),('${other}','Other','tecnico',true,15),('${negative}','Saldo bajo','tecnico',true,0.29);
insert into dotacion_contrato values('${user}','2026-05-22',false,'Confiabilidad'),('${other}',null,false,''),('${negative}',current_date,true,'Técnico');
set test.uid='${admin}';`);
...
await db.exec(fs.readFileSync('supabase/migrations/20260911183000_update_7x7_vacation_business_day_rule.sql','utf8'));
await db.exec(fs.readFileSync('supabase/migrations/20260924163500_allow_negative_vacation_balances_r113.sql','utf8'));
...
await refresh(negative);
await db.exec(`update perfiles set saldo_vacaciones=0.29 where id='${negative}';
update stainher_private.vacation_accrual set baseline_date=(now() at time zone 'America/Santiago')::date where user_id='${negative}';
insert into solicitudes_v15(id,tipo,estado,solicitante_user_id,aprobador_user_id,fecha_inicio,fecha_fin) values ('10000000-0000-0000-0000-000000000002','vacaciones','pendiente','${negative}','${admin}','2026-09-24','2026-09-24');
update solicitudes_v15 set estado='aprobada' where id='10000000-0000-0000-0000-000000000002';`);
const negativeReceipt=await scalar("select vacaciones_saldo_anterior as before,vacaciones_dias_descontados as discounted,vacaciones_saldo_final as after from solicitudes_v15 where id='10000000-0000-0000-0000-000000000002'");
assert.equal(Number(negativeReceipt.before),0.29,'approval records the available balance');
assert.equal(Number(negativeReceipt.discounted),1,'approval keeps the calculated discount');
assert.equal(Number(negativeReceipt.after),-0.71,'approval permits and records a negative balance');
assert.equal(Number((await scalar(`select saldo_vacaciones as balance from perfiles where id='${negative}'`)).balance),-0.71,'profile exposes the negative balance');
console.log('PASS: calendar boundaries, baseline, repeated queries, manual adjustments, missing/future start, inactive/reactivation, permissions, approval, negative balance, historical receipt and restitution.');
await db.close();
