...
(()=>{
  ...
  function suggestions(type=''){
    ...
    ]).map(([tipo,actividad,frecuencia])=>({tipo_intervencion:tipo,actividad,frecuencia}));
  }
  function row(item={}){
  ...
  function enhance(id=''){
    ...
    section.innerHTML=`<div class="stainher-plan-assistant-head"><div><h4>Asistente de Plan Matriz</h4><span class="muted">Define actividades y frecuencias. Las fechas se calendarizan después en Preventivo.</span></div><button class="btn" type="button" data-generate>Generar propuesta</button></div><div class="stainher-plan-assistant-list"></div><button class="btn" type="button" data-add>+ Agregar actividad</button>`;
    ...
    form.onsubmit=async event=>{
      ...
      await window.loadEquipos?.();window.state.v1519PlanCatalogLoaded=false;window.closeModal?.();await window.renderEquipos?.();
  ...
  function installCatalog(){
    ...
    const wrapped=async function(){
      const map=await current.apply(this,arguments);const query=await window.sb.from('plan_matriz_actividades').select('equipo_id,actividad,frecuencia').eq('activo',true);
  ...
  function style(){if(document.getElementById('stainher-plan-assistant-style'))return;const node=document.createElement('style');node.id='stainher-plan-assistant-style';node.textContent=`
    ...
    .stainher-plan-assistant-list{display:grid;gap:9px}.stainher-plan-assistant-row{display:grid;grid-template-columns:minmax(130px,.7fr) minmax(220px,1.6fr) minmax(120px,.7fr) auto;gap:8px;align-items:end}
    .stainher-plan-assistant-row label{min-width:0}.stainher-plan-assistant-row .field{width:100%}
    @media(max-width:760px){.stainher-plan-assistant-head{display:grid}.stainher-plan-assistant-row{grid-template-columns:1fr 1fr}.stainher-plan-assistant-row label:nth-child(2){grid-column:1/-1}.stainher-plan-assistant-row button{align-self:end}}
