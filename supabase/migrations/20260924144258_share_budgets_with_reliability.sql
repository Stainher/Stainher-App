-- R110 · Histórico de presupuestos compartido con Confiabilidad.
-- Confiabilidad puede consultar todo, crear y modificar solo sus presupuestos.
-- La eliminación continúa reservada al Administrador.
...
create policy presupuestos_v1524_admin_reliability_select
...
using (
  exists (
    ...
    where p.id = (select auth.uid())
      and p.activo = true
      and lower(p.rol) in ('administrador', 'confiabilidad')
  )
);

create policy presupuestos_v1524_admin_reliability_insert
on public.presupuestos_tecnico_comerciales_v1524
...
create policy presupuestos_v1524_admin_reliability_update
on public.presupuestos_tecnico_comerciales_v1524
  ...
  exists (
    ...
    where p.id = (select auth.uid())
      ...
      and (
        ...
        or (lower(p.rol) = 'confiabilidad' and created_by = (select auth.uid()))
      )
  )
)
with check (
