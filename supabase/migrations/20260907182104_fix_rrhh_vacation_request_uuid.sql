...
begin
  ...
  if lower(p_accion)='rechazar' then
    ...
    update public.solicitudes_v15
       set estado='rechazada',etapa='finalizada',motivo_rechazo=p_motivo,resuelto_por=v_uid,resuelto_at=now(),finalizada_at=now()
     where id::text=p_id;
    ...
    if r.tipo='vacaciones' then
      ...
      update public.solicitudes_v15
         set estado='pendiente_rrhh',etapa='rrhh',firma_aprobador=p_firma,firmado_aprobador_at=now(),
      ...
      update public.notificaciones_v15
         set descartada=true,resuelta=true,leida=true,leida_at=coalesce(leida_at,now()),
             resuelta_at=coalesce(resuelta_at,now()),resolucion=coalesce(resolucion,'Aprobada por Gerente')
       where referencia_id=p_id and lower(coalesce(modulo_destino,''))='solicitudes' and coalesce(descartada,false)=false;
      ...
      select 'Vacaciones pendientes de firma RR.HH.','Solicitud aprobada jerárquicamente · '||p_id,
             'importante',v_rrhh,v_uid,'solicitudes',p_id
      where not exists(
        select 1 from public.notificaciones_v15 n
         where n.referencia_id=p_id and lower(coalesce(n.modulo_destino,''))='solicitudes'
           and n.destinatario_user_id=v_rrhh and coalesce(n.descartada,false)=false and coalesce(n.resuelta,false)=false
      );
    else
      update public.solicitudes_v15
         set estado='aprobada',etapa='finalizada',resuelto_por=v_uid,resuelto_at=now(),finalizada_at=now()
       where id::text=p_id;
    end if;
    ...
    update public.solicitudes_v15
       set estado='aprobada',etapa='finalizada',firma_rrhh=p_firma,firmado_rrhh_at=now(),rrhh_user_id=v_uid,
    ...
    select s.solicitante_user_id,'vacaciones',s.fecha_inicio,s.fecha_fin,1,'período',
           'Vacaciones aprobadas · Solicitud '||s.id::text,
           extract(year from s.fecha_inicio)::int,extract(month from s.fecha_inicio)::int,v_uid,v_uid,s.id
      from public.solicitudes_v15 s where s.id::text=p_id
    on conflict(solicitud_id) where solicitud_id is not null do nothing;
    update public.notificaciones_v15
       set descartada=true,resuelta=true,leida=true,leida_at=coalesce(leida_at,now()),
           resuelta_at=coalesce(resuelta_at,now()),resolucion=coalesce(resolucion,'Firmada por RR.HH.')
     where referencia_id=p_id and lower(coalesce(modulo_destino,''))='solicitudes' and coalesce(descartada,false)=false;
  else
  ...
  return jsonb_build_object('id',r.id::text,'tipo',r.tipo,'estado',r.estado,'etapa',r.etapa,
    'solicitante_user_id',r.solicitante_user_id,'aprobador_user_id',r.aprobador_user_id,
    'aprobador_rol',r.aprobador_rol,'rrhh_user_id',r.rrhh_user_id);
end;
$$;

revoke all on function public.resolver_solicitud_v1517(text,text,text,text) from public,anon;
grant execute on function public.resolver_solicitud_v1517(text,text,text,text) to authenticated;
