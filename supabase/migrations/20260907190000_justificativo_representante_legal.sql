...
update public.solicitudes_v15 s
set solicitante_nombre = nullif(btrim(p.nombre), '')
from public.perfiles p
where s.tipo = 'justificativo'
  and s.solicitante_user_id = p.id
  and nullif(btrim(s.solicitante_nombre), '') is null;
...
begin
  ...
  return jsonb_build_object(
    ...
    'rrhh_user_id', r.rrhh_user_id, 'justificativo_emitido_at', r.justificativo_emitido_at
  );
end;
$$;
