(function(){
  ...
  function installStyle(){
    ...
    const s=document.createElement('style');s.id='stainher-justificativos-r19-style';s.textContent=`
      ...
      .v1524-letter-preview{padding:20px;border:1px solid var(--line);border-radius:12px;background:#fff;color:#172033;line-height:1.55}.v1524-letter-preview p{margin:0 0 12px}.v1524-letter-preview h4{color:#172033}
  ...
  window.v154RequestModal=async function(...args){
    ...
    form.onsubmit=async event=>{
      ...
      try{
        const q=await window.sb.rpc('crear_justificativo_laboral_v1524',{p_fecha:values.fecha_inicio,p_institucion:values.justificativo_institucion,p_motivo:values.comentario,p_texto:values.justificativo_texto||null});if(q.error)throw q.error;
  ...
  window.v1524BuildJustificationPdf=function(x){
    ...
    y=Math.max(y+10,145);doc.setDrawColor(130,145,165);doc.rect(125,y,70,38);if(x.firma_rrhh)try{doc.addImage(x.firma_rrhh,'PNG',130,y+3,60,25)}catch(_){}const signer=x.rrhh_nombre||(role()==='recursos_humanos'&&String(x.rrhh_user_id||'')===String(window.state?.session?.user?.id||'')?window.state?.profile?.nombre:'Recursos Humanos');doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text(signer||'Recursos Humanos',160,y+31,{align:'center',maxWidth:66});doc.setFont('helvetica','normal');doc.setFontSize(8);doc.text('Recursos Humanos · Stainher Ascensores Ltda.',160,y+35,{align:'center',maxWidth:66});doc.setFontSize(7);doc.text(`Folio: ${String(x.id||'').slice(0,8).toUpperCase()}`,14,275);return doc;
  };
