(function(){
  ...
  window.v154RequestModal=async function(...args){
    ...
    const sync=()=>{
      ...
      if(active){start?.classList.remove('hidden');end?.classList.add('hidden');form.fecha_inicio.required=true;form.fecha_inicio.removeAttribute('min');form.fecha_fin.required=false;form.comentario.placeholder='Ej.: inasistencia a evaluación por encontrarse cumpliendo turno de trabajo.'}
    ...
    form.onsubmit=async event=>{
      ...
      try{
        const q=await window.sb.rpc('crear_justificativo_firmado_v1524',{p_fecha:values.fecha_inicio,p_institucion:values.justificativo_institucion,p_motivo:values.comentario,p_texto:values.justificativo_texto||null,p_firma:null});if(q.error)throw q.error;
  ...
  window.v1524BuildJustificationPdf=function(x){
    ...
    const paragraphs=['De nuestra consideración:',letterText(x),`Antecedente informado: ${x.comentario||'Sin detalle adicional.'}`,x.justificativo_texto?`Información complementaria: ${x.justificativo_texto}`:'',`El presente certificado se extiende a solicitud del interesado, para ser presentado ante ${x.justificativo_institucion||'la institución destinataria'} y acreditar formalmente la circunstancia laboral antes señalada.`,'Sin otro particular, saluda atentamente,'].filter(Boolean);
